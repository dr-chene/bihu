...
public class Data {
    ...
    public static void refreshQuestion(Context context) {
        getSendQuestionPostParams(context);
        Map<String, String> query = new HashMap<>();
        query.put("page", "" + MainActivity.curQuestionPage);
        query.put("count", "" + MainActivity.questionCount);
    ...
    public static void refreshAnswer(Context context, int page, int count, int qid) {
        ...
        query.put("qid", qid + "");
        query.put("token", MainActivity.person.getToken());
        Http http = new Http(context, qid);
    ...
    public static void getQuestionList(Context context, Message msg) {
        try {
            ...
            switch (jsonObject.getInt("status")) {
                ...
                case 200:
                    JSONObject object = jsonObject.getJSONObject("data");
                    MainActivity.totalQuestionCount = object.getInt("totalCount");
                    if (MainActivity.questionCount < MainActivity.totalQuestionCount - 20) {
                        MainActivity.questionCount += 20;
                    } else {
                        MainActivity.questionCount = MainActivity.totalQuestionCount;
                    }
...
//                        int curPage = object.getInt("curPage");
    ...
    private static void getSendQuestionPostParams(Context context) {
        MainActivity.totalQuestionCount = MyHelper.getQuestionCount(context);
        MainActivity.curQuestionPage = MainActivity.totalQuestionCount / MainActivity.questionCount;
    }
}
